...
import { ICompositeKey, IFMEventName, IFkReference, IFmCrudOperations, IFmDispatchOptions, IFmRelationshipOptions, IFmRelationshipOptionsForHasMany, IModel, IRecord, IRecordOptions, IRecordRelationshipMeta, IReduxDispatch } from "../@types/index";
...
export declare class Record<T extends IModel> extends FireModel<T> implements IRecord {
     ...
     */
    static dynamicPathProperties<T extends IModel = IModel>(
    /**
     * the **Model** who's properties are being interogated
     */
    model: new () => T): (string & keyof T)[];
    /**
     * create
     *
     ...
     */
    static create<T extends IModel>(model: new () => T, options?: IRecordOptions): Record<T>;
    /**
     * Creates an empty record and then inserts all values
     * provided along with default values provided in META.
     */
    static local<T extends IModel>(model: new () => T, values: Partial<T>, options?: IRecordOptions & {
     ...
     * @param options
     */
    static add<T extends IModel>(model: (new () => T) | string, payload: T, options?: IRecordOptions): Promise<Record<T>>;
    /**
     * **update**
     *
     ...
     * @param options
     */
    static update<T extends IModel>(model: new () => T, id: string | ICompositeKey<T>, updates: Nullable<Partial<T>>, options?: IRecordOptions): Promise<Record<T>>;
    /**
     ...
     */
    static pushKey<T extends IModel>(model: new () => T, id: string | ICompositeKey<T>, property: keyof T & string, payload: any, options?: IRecordOptions): Promise<string>;
    /**
     * **createWith**
     *
     ...
     */
    static createWith<T extends IModel>(model: new () => T, payload: Partial<T> | string, options?: IRecordOptions): Record<T>;
    /**
     ...
     * @param options
     */
    static get<T extends IModel>(model: new () => T, id: string | ICompositeKey<T>, options?: IRecordOptions): Promise<Record<T>>;
    static remove<T extends IModel>(model: new () => T, id: IFkReference<T>, 
     ...
     */
    static getCompositeKeyFromPath<T extends IModel>(model: new () => T, path: string): IDictionary<any>;
    /**
     ...
     */
    static compositeKey<T extends IModel>(model: new () => T, obj: Partial<T>): ICompositeKey<T>;
    /**
     * Given a Model and a partial representation of that model, this will generate
     ...
     * @param object the data which will be used to generate the Composite key from
     */
    static compositeKeyRef<T extends IModel>(model: new () => T, 
     ...
     * camelCase.
     */
    static modelName<T extends IModel>(model: new () => T): string;
    private _existsOnDB;
     ...
     * produce the "dbPath"
     */
    get dynamicPathComponents(): Array<string & keyof T>;
    /**
     ...
     */
    getMetaForRelationship(property: string & keyof T): IRecordRelationshipMeta;
    /**
