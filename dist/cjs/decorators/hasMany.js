...
function hasMany(fnToModelConstructor, inverse) {
    if (typeof fnToModelConstructor === "string") {
        ...
        if (!model) {
            throw new errors_1.FireModelError(`attempt to lookup "${fnToModelConstructor}" as pre-registered Model failed! ${inverse ? `[ inverse prop was "${inverse}"]` : ""}. The registered models found were: ${modelRegistration_1.listRegisteredModels().join(", ")}`, `firemodel/not-allowed`);
        }
