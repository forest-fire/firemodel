...
function set(target, path, value, options) {
  ...
  const keys = (isArray ? path : split$1(path, opts)).filter(isValidKey);
...
modelProps) {
    ...
    keys.forEach(i => {
        ...
        else if (!fastDeepEqual(from[i], to[i])) {
...
modelRollup) => (modelKlass, key) => {
    ...
    if (modelRollup) {
        ...
        setValue(modelRollup, modelAndProp, {
            ...getValue(modelRollup, modelAndProp),
            ...meta
        });
    }
};
  ...
  ```
    ...
    _getPaths(rec, deltas) {
        const added = (deltas.added || []).reduce((agg, curr) => {
            agg[commonTypes.pathJoin(this.dbPath, curr)] = rec.get(curr);
            return agg;
        }, {});
        const removed = (deltas.removed || []).reduce((agg, curr) => {
            agg[commonTypes.pathJoin(this.dbPath, curr)] = null;
            return agg;
        }, {});
...
class WatchBase {
    ...
    async start(options = {}) {
        ...
        try {
            ...
            (this._dispatcher || FireModel.dispatch)({
                type: exports.FmEvents.WATCHER_STARTING,
                ...watcherItem,
            });
            ...
            await (this._dispatcher || FireModel.dispatch)({
                type: exports.FmEvents.WATCHER_STARTED,
                ...watcherItem,
            });
            return watcherItem;
        }
...
function buildRelationshipPaths(rec, property, fkRef, options = {}) {
    try {
        ...
        const pathToRecordsFkReln = commonTypes.pathJoin(rec.dbPath, // this includes dynamic segments for originating model
        property, 
        ...
        results.push({ path: commonTypes.pathJoin(rec.dbPath, "lastUpdated"), value: now });
        ...
        if (inverseProperty) {
            ...
            const pathToInverseFkReln = fkInverseIsHasManyReln
                ? commonTypes.pathJoin(fkRecord.dbPath, inverseProperty, rec.compositeKeyRef)
                : commonTypes.pathJoin(fkRecord.dbPath, inverseProperty);
            // Inverse paths
            results.push({
            ...
            results.push({
                path: commonTypes.pathJoin(fkRecord.dbPath, "lastUpdated"),
                value: now
            });
        }
...
class Record extends FireModel {
    ...
    static getCompositeKeyFromPath(model, path) {
        ...
        const pathParts = commonTypes.dotNotation(path).split(".");
        const compositeKey = {};
        const segments = commonTypes.dotNotation(r.dbOffset).split(".");
...
async function processHasMany$1(record, rel, config, db) {
                ...
                if (!config.cardinality ||
                    ...
                    if (rel.relType === "hasOne") {
                        ...
                        for (const i of Array(cardinality)) {
                            const fkRec = await processHasMany$1(record, rel, config, db);
                            if (config.relationshipBehavior === "follow") {
                                relnResults.push(fkRec);
                            }
                        }
...
Object.defineProperty(exports, 'fbKey', {
  enumerable: true,
  get: function () {
    return firebaseKey.key;
  }
