...
describe("Client state management", () => {
    ...
    it("when using a RECORD, the localPath is equal to the prefix if there are no dynamic components", async () => {
        const person = await src_1.Record.create(Person_1.Person);
        expect(person.localPath).to.equal(person.localPrefix);
    ...
    it("when using a RECORD, dynamic components from localPrefix are expanded in localPath", async () => {
        ...
        expect(person.localPath).to.equal("/foo/bar/1234");
    });
    it("when using a LIST, the localPath is the prefix, pluralName, and 'all' (assuming no dynamic components)", async () => {
        const people = await src_1.List.all(Person_1.Person);
        expect(people.localPath).to.equal(src_1.pathJoin(people.META.localPrefix, people.pluralName, "all"));
    });
    it("when using a LIST, the localPath is the prefix, pluralName, and then the postFix if it is set", async () => {
        ...
        expect(people.localPath).to.equal(src_1.pathJoin(people.META.localPrefix, people.pluralName, "since"));
    });
});
