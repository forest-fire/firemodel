...
watcherContext) => {
    if (typeof coreDispatchFn !== "function") {
        throw new private_1.FireModelError(`A watcher is being setup but the dispatch function is not a valid function!`, "firemodel/not-allowed");
    }
    // Handle incoming events ...
    return async (event) => {
        const typeLookup = {
            child_added: private_1.FmEvents.RECORD_ADDED,
            child_removed: private_1.FmEvents.RECORD_REMOVED,
            child_changed: private_1.FmEvents.RECORD_CHANGED,
            child_moved: private_1.FmEvents.RECORD_MOVED,
            value: private_1.FmEvents.RECORD_CHANGED,
        };
        let eventContext;
        let errorMessage;
        if (event.kind === "relationship") {
            ...
            if (watcherContext.watcherPaths) {
                ...
                const compositeKey = private_1.Record.getCompositeKeyFromPath(watcherContext.modelConstructor, fullPath);
            ...
            const recordProps = typeof event.value === "object"
                ...
                : { id: event.key };
            const rec = private_1.Record.createWith(watcherContext.modelConstructor, recordProps);
            let type;
            switch (event.kind) {
                case "record":
                    type = event.type;
                ...
                case "server-event":
                    type =
                        event.value === null
                            ? private_1.FmEvents.RECORD_REMOVED
                            : typeLookup[event.eventType];
                    break;
                default:
                    type = private_1.FmEvents.UNEXPECTED_ERROR;
        ...
        private_1.hasInitialized(watcherContext.watcherId);
        return results;
    };
};
