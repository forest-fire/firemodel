...
class DexieDb {
    constructor(_name, ...models) {
        ...
        models.forEach((m) => {
            const r = private_1.Record.create(m);
    ...
    static modelConversion(...modelConstructors) {
        if (modelConstructors.length === 0) {
            throw new private_1.FireModelError(`A call to DexieModel.models() was made without passing in ANY firemodel models into it! You must at least provide one model`, "firemodel/no-models");
        }
        return modelConstructors.reduce((agg, curr) => {
            const dexieModel = [];
            const r = private_1.Record.createWith(curr, new curr());
    ...
    modelIsManagedByDexie(model) {
        const r = private_1.Record.create(model);
    ...
    table(model) {
        ...
        if (!this.modelIsManagedByDexie(model)) {
            throw new private_1.DexieError(`Attempt to get a Dexie.Table for "${private_1.capitalize(r.modelName)}" Firemodel model but this model is not being managed by Dexie! Models being managed are: ${this.modelNames.join(", ")}`, "dexie/table-does-not-exist");
        }
    ...
    record(model) {
        ...
        if (!this.modelNames.includes(r.modelName)) {
            ...
            throw new private_1.DexieError(`Attempt to reach the record API via DexieDb.record("${model}") failed as there is no known Firemodel model of that name. ${isPlural
        ...
        return new private_1.DexieRecord(model, this.table(model), this.meta(r.modelName));
    }
    /**
    ...
    list(model) {
        ...
        return new private_1.DexieList(model, table, meta);
    }
    /**
    ...
    async open() {
        if (this._db.isOpen()) {
            throw new private_1.DexieError(`Attempt to call DexieDb.open() failed because the database is already open!`, `dexie/db-already-open`);
        }
        if (!this.isMapped) {
            this.mapModels();
        }
    ...
    close() {
        if (!this._db.isOpen()) {
            throw new private_1.DexieError(`Attempt to call DexieDb.close() failed because the database is NOT open!`, `dexie/db-not-open`);
        }
        this._db.close();
    }
    _checkPluralThenSingular(obj, name, fn) {
        ...
        throw new private_1.DexieError(`Failed while calling DexieModel.${fn}("${name}") because "${name}" is neither a singular or plural name of a known model!`, `firemodel/invalid-dexie-model`);
    }
    _mapVersionsToDexie() {
