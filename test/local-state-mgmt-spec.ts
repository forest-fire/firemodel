...
describe("Client state management", () => {
  ...
  it(
    "when using a RECORD, the localPath is equal to the prefix plus 'localModelName' if there are no dynamic components",
    async () => {
      const person = await Record.create(Person);
      expect(person.localPath).toBe(person.localPrefix + "/" + person.META.localModelName);
      expect(person.localDynamicComponents).toBeArray();
      expect(person.localDynamicComponents).toHaveLength(0);
    }
  );
  it(
    "when using a RECORD, dynamic components from localPrefix are expanded in localPath",
    async () => {
      const person = await Record.create(DynamicPerson);
      person.id = "1234";
      expect(person.localDynamicComponents).toEqual(expect.arrayContaining(["id"]));
  ...
  it(
    "when using a LIST, the localPath is the prefix, pluralName (assuming no dynamic components)",
    async () => {
      ...
      expect(people.localPath).toBe(pathJoin(people.META.localPrefix, people.pluralName));
      expect(people.localPostfix).toBe("all");
    }
  );

  it(
    "when using a LIST, the localPath is the prefix, pluralName, and then the postFix if it is set",
    async () => {
